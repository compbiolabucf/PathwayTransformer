...
After data preprocessing is complete, 'entry,py' script can be run to train and test the Pathway-Transformer model for a particular pathway. Note that, the root dataset directory must be set in the stated script to conduct experimentation for a particular pathway.
