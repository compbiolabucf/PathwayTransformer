...
class Batch():
    ...
    def to(self, device):
        # print("qibing_log_collator_to: ", device)
        # print("qibing index0", self.idx)
        self.idx = self.idx.to(device)
        # print("qibing index1", self.idx)
...
def collator(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20):
    # print("qibing_log_collator1: ", items)

    # for item in items:
        # print("qibing_log_collator1: ", item.x.size(0), max_node)

    items = [item for item in items if item is not None and item.x.size(0) <= max_node]
    # print("qibing_log_collator2: ", items[0].x.size(0), max_node)
    ...
    # print("qibing_log_collator3: ", items)

    idxs, attn_biases, attn_edge_types, spatial_poses, in_degrees, out_degrees, xs, edge_inputs, ys = zip(
        *items)
    # print("qibing_log_collator4: ", "qibing_idxs:", idxs, "Qibing_attn_biases:", attn_biases, "qibing attn_edge_types: ", attn_edge_types, "spatial_poses", spatial_poses, "in_degrees: ", in_degrees, "out_degrees: ", out_degrees, "xs: ", xs, "edge_inputs: ", edge_inputs, "ys: ", ys)
